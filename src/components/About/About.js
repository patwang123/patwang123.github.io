import './About.css';
export default function About () {
    return (
        <div id='about-me' className='about-me'>
            <h1 className='about-title'>
                About me
            </h1>
            <p>
                I'm Patrick Wang, a sophomore at UC Berkeley, class of 2023. Nice to meet you! As of now, I am looking for an intern position in a variety of technical fields in CS and math, varying from software engineering to machine learning to quant trading. If you want to learn more about me or contact me, check out my social links above!
            </p>
            <p>
                This site's still under construction (I started like 3 days ago...), but I've been putting my foot on the pedal, so hopefully this will mostly be done by the end of November (doable in 2 days?). Anyways, feel free to look around, my social links (Github, Linkedin, resume, etc) are working but everything under may be incomplete. 
            </p>
        </div>
    )
}